...
sequenceDiagram
    ...
    activate server
    Note right of browser: The browser sends the new note data in the POST request body
    ...
    Note right of browser: The browser executes the JavaScript code that fetches JSON data from the server

    browser->>server: GET [https://studies.cs.helsinki.fi/exampleapp/data.json](https://studies.cs.helsinki.fi/exampleapp/data.json)
    activate server
    server-->>browser: [{ "content": "HTML is easy", "date": "2023-1-1" }, ..., { "content": "my new note", "date": "2026-08-18" }]
    deactivate server
